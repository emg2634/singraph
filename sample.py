...
for radius in tqdm(radiuses, desc='Calculating max-min potentials'):
        ...
        for y_move in np.arange(0, atom_lattice, 0.1):
            ...
            for x, y in tip_base:
                if np.sqrt((x) ** 2 + (y) ** 2) <= radius + 0.001:  # 만약 tip 원자가 반지름 안에 속해 있다면
                    potential_sum += potential_at_xy(x + x_move, y + y_move, B,
                                                     λ)  # potential_sum에 해당 tip 원자의 potential을 더한다.
    ...
    potential_barrier.append(
        max_z_value_sum - min_z_value_sum)  # potential_barrier에 max_z_value_sum과 min_z_value_sum의 차이를 저장
...
plt.xticks(np.arange(0,atom_limit,6))
